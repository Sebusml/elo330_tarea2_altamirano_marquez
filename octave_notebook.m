...
if (size(index_max,2)>1)
	...
	for i = 2:size(index_max,2)
		...
		elseif (first_index>2 && last_index<size(x))
			y=polyfit([first_index-2 first_index-1 last_index+1 last_index+2],[x(first_index-2) sat(first_index-1) sat(last_index+1) sat(last_index+2)],2);
...
if (size(index_max,2)>1)
	...
	for i = 2:size(index_min,2)
		...
		elseif (first_index>2 && last_index<size(x,2)-1)
			y=polyfit([first_index-2 first_index-1 last_index+1 last_index+2],[x(first_index-2) sat(first_index-1) sat(last_index+1) sat(last_index+2)],2);
	 		z=polyval(y,first_index:last_index);
	 		x(first_index:last_index)=z;
